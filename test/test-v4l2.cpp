...
int main(int argc, char** argv) {
    ...
    ioctl_safe(fd, VIDIOC_QUERYCAP, &cap);
    ...
    ioctl_safe(fd, VIDIOC_S_FMT, &fmt);
    ...
    ioctl_safe(fd, VIDIOC_REQBUFS, &req);
    if (req.count < 2) {
    ...
    for (bufferNum = 0; bufferNum < req.count; ++bufferNum) {
        ...
        buf.index = bufferNum;
        ioctl_safe(fd, VIDIOC_QUERYBUF, &buf);
    ...
    for (int i = 0; i < bufferNum; ++i) {
        ...
        buf.memory = V4L2_MEMORY_MMAP;
        buf.index = i;
        ioctl_safe(fd, VIDIOC_QBUF, &buf);
    }

    // stream on
    enum v4l2_buf_type type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    ioctl_safe(fd, VIDIOC_STREAMON, &type);
    ...
    for (int i = 0; i < 20; i++) {
        ...
        buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        buf.memory = V4L2_MEMORY_MMAP;
        ioctl_safe(fd, VIDIOC_DQBUF, &buf);
    ...
    ioctl_safe(fd, VIDIOC_STREAMOFF, &type);
